(function() {
    
    var SubItemsController = function ($scope, $routeParams) {
        var categoryId = $routeParams.categoryId;
        $scope.items = null;
        $scope.categories = null;
        
        function init() {
            //Search the categories for the categoryId
            for (var i=0,len=$scope.categories.length;i<len;i++) {
               if ($scope.categories[i].id === parseInt(categoryId)) {
	               //Items must go before categories....??????
			   	   $scope.items = $scope.categories[i].items;
                   $scope.categories = $scope.categories[i];
                   console.log(i +"testing");
                   break;
               }
            }
            
        }
        	
$scope.categories= [
      		{
	      		id: 1,
      			title: 'Tools',
      			location: 'Garage',
      			icon: 'wrench',
      			counter: 0,
      			items: [
      				{
	      			id: 1,
	      			title: 'Craftsman Tool Storage',
	      			desc: '11 Drawer Tool Chest',
	      			price: 299,
	      			datePurch: '2003-03-17',
	      			qty: 1,
	      			value: '299 '	
		  			},
		  			{
			  		id: 2,
			  		title: 'Air Compressor',
			  		desc: 'Portable 8gal Electric',
			  		price: 179,
			  		datePurch: '2003-10-19',
			  		qty: 1,
			  		value: 150
		  			}
		  			]
	  		},
	 		{
		 		id: 2,
      			title: 'Electronics',
      			location: 'Home',
      			icon: 'off',
      			counter: 0,
      			items: [
      				{
	      			id: 3,
	      			title: 'Samsung HD TV',
	      			desc: '52in LED',
	      			price: 2500,
	      			datePurch: '2011-02-11',
	      			qty: 1,
	      			value: '    '	
		  			},
		  			{
			  		id: 4,
			  		title: 'Playstation 4',
			  		desc: '',
			  		price: 399,
			  		datePurch: '2013-10-12',
			  		qty: 1,
			  		value: 399
		  			}
		  			]
	  		},
	  		{
		  		id: 3,
		  		title: 'Furniture',
		  		location: 'Home',
		  		icon: 'book',
		  		counter: 0,
		  		items: [
		  			{
			  		id: 5,
			  		title: 'BookCase with doors',
			  		desc: '',
			  		price: 150,
			  		datePurch: '2013-05-05',
			  		qty: 1,
			  		value: ''
		  			},
		  			{
			  		id: 6,
			  		title: 'End Table with drawer',
			  		desc: '',
			  		price: 99,
			  		datePurch: '',
			  		qty: 4,
			  		value: ''
		  			},
		  			{
			  		id: 7,
			  		title: 'Queen Matress & Boxspring',
			  		desc: 'includes bed frame and headboard',
			  		price: 9500,
			  		datePurch: '',
			  		qty: 1,
			  		value: ''		  			
			  		}
			  		]	
	  		},
	  		{
		  		id: 4,
		  		title: 'Decorations',
		  		location: 'Home',
		  		icon: 'picture',
		  		counter: 0,
		  		items: [
		  			{
			  		id: 8,
			  		title: 'Large Mirror',
			  		desc: 'various sizes and styles',
			  		price: 99,
			  		datePurch: '',
			  		qty: 3,
			  		value: ''
		  			},
		  			{
			  		id: 9,
			  		title: 'Clock',
			  		desc: '',
			  		price: 200,
			  		datePurch: '2002-07-04',
			  		qty: 2,
			  		value: ''
		  			}
		  			]

    		},
	 		{
		 		id: 5,
      			title: 'Barn',
      			location: 'Backyard',
      			icon: 'tree-deciduous',
      			counter: 0,
      			items: [
      				{
	      			id: 10,
	      			title: 'Christmas Tree & Lights',
	      			desc: 'Holiday Decorations, etc.',
	      			price: 450,
	      			datePurch: '1995-11-15',
	      			qty: 1,
	      			value: 'priceless' 	
		  			},
		  			{
			  		id: 11,
			  		title: 'Mountain Bikes',
			  		desc: 'from that summer we got into bike riding',
			  		price: 300,
			  		datePurch: '2002-04-29',
			  		qty: 2,
			  		value: 300
		  			}
		  			]
	  		},
	 		{
		 		id: 6,
      			title: 'Checked Out',
      			location: 'Work',
      			icon: 'inbox',
      			counter: 0,
      			items: [
      				{
	      			id: 12,
	      			title: 'iPad mini',
	      			desc: 'PROPERTY OF RON SWANSON',
	      			price: 0,
	      			datePurch: '2013-01-15',
	      			qty: 1,
	      			value: 550	
		  			},
		  			{
			  		id: 13,
			  		title: 'Label Maker',
			  		desc: 'Borrowed from Dwight',
			  		price: 0,
			  		datePurch: '',
			  		qty: 1,
			  		value: 79,
		  			}
		  			]
	  		}    
        ];
               
        
        
        init();
    };
    
    SubItemsController.$inject = ['$scope', '$routeParams'];

    angular.module('sortListApp')
      .controller('SubItemsController', SubItemsController);
    
}());